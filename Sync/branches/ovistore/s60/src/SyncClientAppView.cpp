...
CCoeControl* CSyncClientAppView::ComponentControl(TInt /*aIndex*/) const
	{
	if (iProfiles > 0) {
		DP("ComponentControl returning settings list");
		iItemList->MakeVisible(ETrue);
		iEmptyListbox->MakeVisible(EFalse);
		return iItemList;
	} else {
		DP("ComponentControl returning empty list");
		iItemList->MakeVisible(EFalse);
		iEmptyListbox->MakeVisible(ETrue);
		return iEmptyListbox;
	}
	}

void CSyncClientAppView::CreateEmptyListbox()
	{
	iEmptyListbox = new (ELeave) CAknSingleLargeStyleListBox;
	iEmptyListbox->ConstructL(this);//, aListboxFlags);
	iEmptyListbox->SetMopParent( this );
	iEmptyListbox->SetContainerWindowL(*this);
//	iEmptyListbox->CreateScrollBarFrameL(ETrue);
//	iEmptyListbox->ScrollBarFrame()->SetScrollBarVisibilityL(CEikScrollBarFrame::EAuto, CEikScrollBarFrame::EAuto );
	iEmptyListbox->SetSize(Rect().Size());
	iEmptyListbox->MakeVisible(EFalse);
	HBufC *text = iEikonEnv->AllocReadResourceL(R_NO_PROFILES);
	iEmptyListbox->View()->SetListEmptyTextL(*text);
	delete text;
	}
	

void CSyncClientAppView::SizeChanged()
    {
	DP2("SizeChanged, w=%d, h=%d", Size().iWidth, Size().iHeight);
	iEmptyListbox->SetSize(Size());
	iEmptyListbox->SizeChanged();
	ActivateL();  		
	DrawNow();
    }
